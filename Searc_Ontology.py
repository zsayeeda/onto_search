...
onto = get_ontology("file://" + path + "/ontology19.owl").load()
...
def Individuals(direct_parent):
  ...
  print("Individuals are returning all the compounds:{}".format(compound_names))
...
# load is done at the beginning and the variable is "onto"\
  ...
  term1_control = False
  term2_control = False
  converted_direct_parent = ClassName(direct_parent)
  compounds_in_chemical_class = Individuals(converted_direct_parent)
  print("passing to Individuals:{}".format(converted_direct_parent))
  ...
  for compound in compounds_in_chemical_class:
    ...
    if round(float(m_w),3) >= float(molecular_weight_from) and  round(float(m_w),3) <= float(molecular_weight_to):
      m_w_control = True
    ...
    if round(float(logp),3) >= float(logp_from) and  round(float(logp),3) <= float(logp_to) :
      ...
      logp_control = False
      # print("logp control set to:{}".format(logp_control))
    converted_sub_ontology1 = ClassName(sub_ontology1)
    converted_sub_ontology2 = ClassName(sub_ontology2)
    term1 = term_sub_ontology1
    term2 = term_sub_ontology2
    ...
    if m_w_control == True and logp_control == True and term1_control == True:
      ...
      total_sub_ont_from_input.append(sub_ontology1)
      total_sub_ont_from_input.append(sub_ontology2)
